...
Both frameworks target iOS and Android. React Native's Android support is well-established and covers the full component surface. ActionUI's Android renderer (Jetpack Compose) is in active development: 56 of 64 element types are ported, with the full navigation stack, modals (window- and element-level), dialogs, dynamic loading, an embedded WebView, async images, and a Map element working (shipped as an optional self-registering provider module - an OpenStreetMap-backed one with no dependencies or API keys - so apps that show no map carry no map engine). Some elements (Canvas, VideoPlayer) and some modifiers (animation system, element-level popover) are not yet ported. For apps that can work within the current element coverage, the JSON is the same — no code changes between platforms.
