...
#
# Shell conventions (see memory: shell-scripting-prefs):
#   - no `set -e`; every fallible step is checked and reports a clear error
#   - external tools are invoked by absolute path
#   - tools are never called in an `if` condition (call first, check $? next)
