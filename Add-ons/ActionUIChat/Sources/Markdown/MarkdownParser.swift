// Add-ons/ActionUIChat/Sources/Markdown/MarkdownParser.swift
//
// A small, dependency-free Markdown parser for the Chat transcript (M2). No third-party
// package: a line-based block parser plus a recursive inline parser, covering the subset an
// AI / chat transcript actually uses:
//
//   blocks:  ATX headings, paragraphs, fenced code (``` / ~~~), block quotes, ordered /
//            unordered (and nested) lists, thematic breaks, GFM pipe tables.
//   inline:  **strong**, *emphasis*, ***both***, ~~strikethrough~~, `code`, [links](url),
//            backslash escapes, and hard line breaks (two trailing spaces or a backslash).
//
// The parser is total: malformed or INCOMPLETE input never throws and never loses text -
// an unterminated emphasis / code span / link renders as the literal delimiter text. That
// property is what makes streaming safe (a half-typed `**bold` shows as plain "**bold"
// until the closing delimiter streams in), with the one destructive case - an open code
// fence - handled by the renderer balancing it (see MarkdownView).
...
enum MarkdownBlock: Equatable {
    ...
    case list(ordered: Bool, start: Int, tight: Bool, items: [[MarkdownBlock]])
    case thematicBreak
    case table(headers: [[MarkdownInline]], alignments: [MarkdownColumnAlignment], rows: [[[MarkdownInline]]])
}
...
enum MarkdownParser {
    ...
    private static func parseFence(_ lines: [String], _ i: inout Int) -> MarkdownBlock? {
    ...
    private static func parseHeading(_ line: String) -> MarkdownBlock? {
        ...
        return .heading(level: level, MarkdownInlineParser.parse(content))
    }
    ...
    private static func parseBlockQuote(_ lines: [String], _ i: inout Int) -> MarkdownBlock {
        var inner: [String] = []
        while i < lines.count {
            let line = lines[i]
    ...
    private struct ListMarker {
        let ordered: Bool
        let start: Int
        let indent: Int          // leading spaces before the marker
        let contentIndent: Int   // column where the item content begins
        let content: String
    }
    ...
    private static func parseList(_ lines: [String], _ i: inout Int) -> MarkdownBlock {
        ...
        while i < lines.count {
            ...
            while i < lines.count {
                ...
                if lead >= contentIndent {
                    ...
                    itemLines.append(line)   // lazy paragraph continuation
                    i += 1
                }
            }
    ...
    private static func parseTable(_ lines: [String], _ i: inout Int) -> MarkdownBlock? {
        guard lines[i].contains("|"), (i + 1) < lines.count, isTableDelimiter(lines[i + 1]) else {
            return nil
        }
        let headers = splitTableRow(lines[i]).map { MarkdownInlineParser.parse($0) }
        ...
        while j < lines.count {
            ...
            rows.append(splitTableRow(line).map { MarkdownInlineParser.parse($0) })
            j += 1
        }
        i = j
    ...
    private static func parseAlignments(_ line: String) -> [MarkdownColumnAlignment] {
    ...
    private static func parseParagraph(_ lines: [String], _ i: inout Int) -> MarkdownBlock {
        ...
        return .paragraph(MarkdownInlineParser.parse(joinParagraph(collected)))
    }

    // Joins soft-wrapped paragraph lines: a normal wrap becomes a space; a line ending in two
    // or more spaces or a backslash becomes a hard break (U+2028, which the inline parser maps
    // to .lineBreak).
...
private struct MarkdownInlineParser {
    ...
    static func parse(_ text: String) -> [MarkdownInline] {
        var parser = MarkdownInlineParser(text)
        return parser.parseRuns(stop: nil).nodes
    }

    // Parses inline nodes until end of input, or until `stop` (a delimiter run) is found at the
    // top level - in which case `stop` is consumed and `matched` is true. Used recursively for
    // emphasis content (stop = the closing delimiter) and link text (stop = "]").
    private mutating func parseRuns(stop: String?) -> (nodes: [MarkdownInline], matched: Bool) {
    ...
    private mutating func parseEmphasis(_ marker: Character) -> MarkdownInline? {
        let start = pos
        var run = 0
