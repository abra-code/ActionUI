// Add-ons/ActionUIChat/Tests/Core/ChatTranscriptTests.swift
//
...
private final class HistoryTestLogger: ActionUILogger {
...
/// Collects the JSON envelopes fired to a entryActionID (populated from a @Sendable handler).
...
final class ChatTranscriptSeamTests: XCTestCase {
    ...
    func testEntryFiresOncePerFinalizedEntryWithMonotonicSequence() {
        let sink = EntrySink()
        let actionID = "test.history.\(UUID().uuidString)"
        ActionUIModel.shared.registerActionHandler(for: actionID) { _, _, _, _, context in
            if let json = context as? String {
                sink.add(json)
            }
        }
        defer { ActionUIModel.shared.unregisterActionHandler(for: actionID) }

        let store = makeStore(properties: ["entryActionID": actionID])
    ...
    func testNoEntryActionMeansNoFiring() {
        // Without entryActionID configured, driving a turn must not crash / fire anything.
        let store = makeStore()
        store.route(.messageStart(itemID: "a1", role: .agent))
...
// MARK: - Config parsing (readOnly / content / entryActionID)
...
final class ChatContentRestoreTests: XCTestCase {
    ...
    func testToolCallEntryReFiresSoTrailingOutputIsPersisted() {
        let sink = EntrySink()
        let actionID = "test.entry.tool.\(UUID().uuidString)"
        ActionUIModel.shared.registerActionHandler(for: actionID) { _, _, _, _, context in
            if let json = context as? String { sink.add(json) }
        }
        defer { ActionUIModel.shared.unregisterActionHandler(for: actionID) }

        let source = FakeContentSource()
        let store = makeStore(properties: ["entryActionID": actionID], source: source)
        store.start()
