// Add-ons/ActionUIDiff/Sources/DiffView/DiffModel.swift
//
...
//
// STANDALONE BY DESIGN: this module (DiffView) imports Foundation only - no Chat
// types, no ActionUI - and the extraction has happened: this package IS the
// standalone component (the way RichText and AsyncImageCache were extracted).
// ActionUIChat consumes the DiffView product for its tool-card diffs, and the
// Diff element (Sources/ActionUIDiff) wraps the same view for JSON documents -
// the way Chat message bodies wrap RichText. The API here is public so both
...
public struct DiffLine: Identifiable, Equatable, Sendable {
    ...
    public let oldNumber: Int?     // 1-based line number in the old text (context / removed)
    public let newNumber: Int?     // 1-based line number in the new text (context / added)
...
public struct DiffHunk: Identifiable, Equatable, Sendable {
    ...
    public init(id: Int, lines: [DiffLine], skippedBefore: Int) {
        self.id = id
        self.lines = lines
...
public struct DiffResult: Equatable, Sendable {
    ...
    public let trailingSkippedLines: Int   // unchanged lines collapsed after the last hunk
    public let isIdentical: Bool
    public let exceedsSizeLimit: Bool

    public init(hunks: [DiffHunk], trailingSkippedLines: Int, isIdentical: Bool, exceedsSizeLimit: Bool) {
        self.hunks = hunks
        self.trailingSkippedLines = trailingSkippedLines
        self.isIdentical = isIdentical
        self.exceedsSizeLimit = exceedsSizeLimit
    }
}

public enum DiffComputer {
    ...
    public static func compute(oldText: String, newText: String,
