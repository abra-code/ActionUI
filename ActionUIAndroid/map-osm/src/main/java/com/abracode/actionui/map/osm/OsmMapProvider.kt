...
/**
 * Self-registration hook: registers [OsmMapView] as the `Map` element when
 * the app process starts, so a client only has to LINK this module - no
 * code, no manual `ActionUIRegistry.register` call.
 *
 ...
 */
class OsmMapProvider : ContentProvider() {
