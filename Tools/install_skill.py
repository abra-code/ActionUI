#!/usr/bin/env python3
"""
ActionUI Skill Installer
========================
Deploy a built skill flavor to a target location.

The `claude` flavor installs as a Claude Code skill directory (SKILL.md + scripts + docs).
...
def install_claude(src: Path, dest_root: Path, force: bool) -> None:
    ...
    print(f"  • The validator is bundled at {target}/scripts/validate_actionui.py.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Install a built ActionUI skill flavor to a target location.",
    ...
    parser.add_argument(
        ...
        help="Destination root for claude flavor (defaults to current directory). "
             "The skill is written to <dest>/.claude/skills/actionui/.",
    )
    parser.add_argument(
        "--user",
        action="store_true",
        help="Install claude flavor at user level (~/.claude/skills/actionui/).",
    )
    parser.add_argument(
        "--out",
        type=Path,
