// Tests/Views/LazyVGridTemplateTests.swift
/*
 LazyVGridTemplateTests.swift

 Tests for LazyVGrid data-driven template rendering: each data row set via
 setElementRows becomes one grid cell, flowing into the declared columns.
*/

import XCTest
import SwiftUI
@testable import ActionUI

@MainActor
final class LazyVGridTemplateTests: XCTestCase {
    private var logger: XCTestLogger!
    private var windowUUID: String!

    override func setUp() async throws {
        try await super.setUp()
        logger = XCTestLogger(maxLevel: .verbose)
        ActionUIRegistry.shared.setLogger(logger)
        ActionUIModel.shared.logger = logger
        ActionUIRegistry.shared.resetForTesting()
        ActionUIModel.resetForTesting()
        windowUUID = UUID().uuidString
    }

    override func tearDown() async throws {
        ActionUIRegistry.shared.resetForTesting()
        ActionUIModel.resetForTesting()
        logger = nil
        windowUUID = nil
        try await super.tearDown()
    }

    func testLazyVGridTemplate_decodesTemplateSubview() throws {
        let json = """
        {
            "type": "LazyVGrid",
            "id": 10,
            "properties": { "columns": [ { "flexible": true }, { "flexible": true } ] },
            "template": { "type": "Text", "properties": { "text": "$1" } }
        }
        """
        let element = try load(json)
        let template = element.subviews?["template"] as? ActionUIElement
        XCTAssertNotNil(template, "template subview should be decoded")
        XCTAssertEqual(template?.type, "Text")
        XCTAssertEqual(template?.properties["text"] as? String, "$1")
    }

    func testLazyVGridTemplate_initializesContentState() throws {
        let json = """
        {
            "type": "LazyVGrid",
            "id": 10,
            "template": { "type": "Text", "properties": { "text": "$1" } }
        }
        """
        let element = try load(json)
        guard let viewModel = ActionUIModel.shared.windowModels[windowUUID]?.viewModels[element.id] else {
            XCTFail("viewModel not found"); return
        }
        let content = viewModel.states["content"] as? [[String]]
        XCTAssertNotNil(content, "states[\"content\"] should be initialized as [[String]]")
        XCTAssertTrue(content!.isEmpty, "content should start empty")
    }

    func testLazyVGridTemplate_buildsViewWithRows() throws {
        let json = """
        {
            "type": "LazyVGrid",
            "id": 10,
            "properties": { "columns": [ { "minimum": 80 }, { "flexible": true } ] },
            "template": { "type": "Text", "properties": { "text": "$1" } }
        }
        """
        let element = try load(json)
        guard let viewModel = ActionUIModel.shared.windowModels[windowUUID]?.viewModels[element.id] else {
            XCTFail("viewModel not found"); return
        }

        viewModel.states["content"] = [["Alpha"], ["Beta"], ["Gamma"]]

        let validatedProps = LazyVGrid.validateProperties(element.properties, logger)
        let view = ActionUIRegistry.shared.buildView(
            for: element, model: viewModel,
            windowUUID: windowUUID,
            validatedProperties: validatedProps
        )
        XCTAssertFalse(view is SwiftUI.EmptyView, "Should render a LazyVGrid, not EmptyView")
    }

    func testLazyVGridTemplate_buildsViewWithNoRows() throws {
        let json = """
        {
            "type": "LazyVGrid",
            "id": 10,
            "template": { "type": "Text", "properties": { "text": "$1" } }
        }
        """
        let element = try load(json)
        guard let viewModel = ActionUIModel.shared.windowModels[windowUUID]?.viewModels[element.id] else {
            XCTFail("viewModel not found"); return
        }

        let validatedProps = LazyVGrid.validateProperties(element.properties, logger)
        let view = ActionUIRegistry.shared.buildView(
            for: element, model: viewModel,
            windowUUID: windowUUID,
            validatedProperties: validatedProps
        )
        // Empty rows: the grid renders with no cells, still a LazyVGrid (not EmptyView)
        XCTAssertFalse(view is SwiftUI.EmptyView)
    }

    // MARK: - Helpers

    private func load(_ jsonString: String) throws -> ActionUIElement {
        let data = Data(jsonString.utf8)
        return try ActionUIModel.shared.loadDescription(from: data, format: "json", windowUUID: windowUUID)
    }
}
